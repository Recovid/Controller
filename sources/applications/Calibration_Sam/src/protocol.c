...
bool send_INIT(const char* information)
{
    ...
    *curr = '\0';
    ...
    return send(INIT_frame)
        && send_SET(VT___, VT____FMT, g_handler->get_setting_VT_mL()         )
        && send_SET(FR___, FR____FMT, g_handler->get_setting_FR_pm()         )
        && send_SET(PEP__, PEP___FMT, g_handler->get_setting_PEP_cmH2O()     )
        && send_SET(VMAX_, VMAX__FMT, g_handler->get_setting_Vmax_Lpm()      )
        && send_SET(EoI__, EoI___FMT, g_handler->get_setting_EoI_ratio()*10  )
        && send_SET(TPLAT, TPLAT_FMT, g_handler->get_setting_Tplat_ms()      )
        && send_SET(VTMIN, VTMIN_FMT, g_handler->get_setting_VTmin_mL()      )
        && send_SET(VTMAX, VTMAX_FMT, g_handler->get_setting_VTmax_mL()      )
        && send_SET(PMAX_, PMAX__FMT, g_handler->get_setting_Pmax_cmH2O()    )
        && send_SET(PMIN_, PMIN__FMT, g_handler->get_setting_Pmin_cmH2O()    )
        && send_SET(FRMIN, FRMIN_FMT, g_handler->get_setting_FRmin_pm()      )
        && send_SET(VMMIN, VMMIN_FMT, g_handler->get_setting_VMmin_Lm()     );
}
...
void send_and_recv()
{
    ...
    } else if(state == FRAME_COMPLETE)
    {
        ...
        else if ((pl = payload(frame, SET_)) && g_handler!=NULL) {
            bool process_result =
                process(&pl,  0, VT___, VT____FMT, g_handler->set_setting_VT_mL) ||
                process(&pl,  1, FR___, FR____FMT, g_handler->set_setting_FR_pm) ||
                process(&pl,  2, VMAX_, VMAX__FMT, g_handler->set_setting_Vmax_Lpm) ||
                process(&pl,  3, EoI__, EoI___FMT, set_setting_EoI_ratio_x10) ||
                process(&pl,  4, PEP__, PEP___FMT, g_handler->set_setting_PEP_cmH2O) ||
                process(&pl, -1, TPLAT, TPLAT_FMT, NULL) ||
                process(&pl,  5, VTMIN, VTMIN_FMT, g_handler->set_setting_VTmin_mL) ||
                process(&pl,  6, VTMAX, VTMAX_FMT, g_handler->set_setting_VTmax_mL) ||
                process(&pl,  7, PMAX_, PMAX__FMT, g_handler->set_setting_Pmax_cmH2O) ||
                process(&pl,  8, PMIN_, PMIN__FMT, g_handler->set_setting_Pmin_cmH2O) ||
                process(&pl,  9, FRMIN, FRMIN_FMT, g_handler->set_setting_FRmin_pm) ||
                process(&pl, 10, VMMIN, VMMIN_FMT, g_handler->set_setting_VMmin_Lpm);
            UNUSED(process_result)
        }
        else if ((pl = payload(frame, PINS)) && g_handler!=NULL) {
            process_uint16(&pl, -1, "", P_FMT, g_handler->set_command_Tpins_ms);
        }
        else if ((pl = payload(frame, PEXP)) && g_handler!=NULL) {
            process_uint16(&pl, -1, "", P_FMT, g_handler->set_command_Tpexp_ms);
        }
        else if ((pl = payload(frame, PBIP)) && g_handler!=NULL) {
            process_uint16(&pl, -1, "", P_FMT, g_handler->set_command_Tpbip_ms);
        }
        else if ((pl = payload(frame, SRST)) && g_handler!=NULL) {
            g_handler->set_command_soft_reset();
        }
        else {
            dbg_printf("%s", frame); // Unknown
        }
