...
int hmi_read_data(char * data, uint16_t data_size)
{
    uint16_t count = 0;
...
bool hmi_write_data(const char * data, uint16_t data_size)
{
    __disable_irq();
...
bool hmi_uart_init()
{
    ...
    HAL_UART_RegisterCallback(&hmi_uart, HAL_UART_RX_COMPLETE_CB_ID, hmi_RxCpltCallback);
    HAL_UART_RegisterCallback(&hmi_uart, HAL_UART_TX_COMPLETE_CB_ID, hmi_TxCpltCallback);
    HAL_UART_RegisterCallback(&hmi_uart, HAL_UART_ERROR_CB_ID, hmi_ErrorCallback);
